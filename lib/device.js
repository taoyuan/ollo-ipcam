...
Camera.prototype.write = function(data) {
  ...
  var self = this;
  var postOptions = {
    host:this._cloudOpts.stream.host,
    port:this._cloudOpts.stream.port,
    path:'/rest/v0/camera/'+this._guid+'/snapshot',
    method:'POST'
  };

  var proto = (this._cloudOpts.stream.port==443) ? https:http
  var getReq = http.get(this._cameraOpts,function(getRes) {

    postOptions.headers = getRes.headers;
    postOptions.headers['X-Ollo-Token'] = self._token
